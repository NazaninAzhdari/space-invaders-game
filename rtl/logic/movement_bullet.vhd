        ...
        process(i_clk) is
            begin 
                if rising_edge(i_clk) then
                    ...
                    if i_reset = '1' then
                        ------------------------------------------------------------------
                        --The whole loop happens in only one clock cycle, but in sequence.
                        ...
                        if i_en = '1' then
                            ---------------------------------------------------------------------------------------------------------------
                            --If button gets pressed and released, the loop starts and it finnishes in one clock cycle, but in sequence.
                            --When the first inactive bullet gets activated, the loop ends, BCS of exit statement.
                            --So only one bullet will be fired by the fulling edge of the bullet button.
                            --If all the bullets are active, nothing will be fired.
                            --NB! if instead of falling edge of button, i only check i_bullet_btn = '1' the logic will not work as expected!
                            --Reason: BCS if you press the button, the button will be high for more than one clock cycle.
                            --Since each bullet will be fired at one clock cycle.
                            --So all the bullets will be fired at once by pressing the button.
                            --Since they have been fireed by one clock cycle apart, their x and y would be the same.
                            ...
                            if r_counter < pc_BULLET_SPEED then
                                ...
                                --In the clock cycle that r_counter = pc_BULLET_SPEED, all the active bullets will be moved upward by one.
                                --In the next clock cycle r_counter becomes zero. 
                                --The r_counter increases by one in each clock cycle and 
                                --When it has reached to pc_BULLET_SPEED, the whole loop happens again.
                                -----------------------------------------------------------------------------------------------------------
                                for i in 0 to pc_BULLET_LIMIT-1 loop
                                    if r_bullets(i).ACTIVE = '1' then
                                        ...
                                        else --if bullet reachs the top of the screen, the bullet becomes inactive and will be located in initial state.
                                            r_bullets(i).Y <= pc_Y_START_BULLET;
                                            r_bullets(i).ACTIVE <= '0';
                                        end if;
                                    end if;
                        ...
                        -----------------------------------------------------------------
                        --If the bullet collides with Invaders, it will be killed.
                        --So it becomes Inactive and ready to be fired again.
                        -----------------------------------------------------------------
                        for i in 0 to pc_BULLET_LIMIT -1 loop
                    ...
                    end if; --end of i_reset = '1' or else
